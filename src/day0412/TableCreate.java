...
public class TableCreate extends JFrame implements ActionListener {
	...
	private void setTableName(String tName) {
		if(createQuery.toString().contains("create")) {
			JOptionPane.showMessageDialog(this, "create쿼리가 이미 생성되어있습니다. 삭제 후 다시 만들어주세요");
		return;
		}//if
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if(ae.getSource()==jbtnTableName) {
			...
			if("".equals(jtfTableName.getText().trim())) {
				JOptionPane.showMessageDialog(this,"테이블명은 필수 입력입니다.");
				return;
			}//if
		...
		if(ae.getSource()==jbtnSetCol) {
			...
			JOptionPane.showMessageDialog(this,"테이블명을 설정한 후 컬럼을 추가할 수 있습니다.");
			...
			if("".equals(jtfSize.getText().trim())) {
				JOptionPane.showMessageDialog(this,"컬럼크기는 필수 입력입니다.");
		...
		if(jcbNN.isSelected()&&jcbPmKey.isSelected()) {
				jcbNN.setSelected(false);//Primary key인 상태에서는 N.N을 해제한다.
		}//if
		...
		if(ae.getSource()==jbtnCreate) {
			...
			try {
				...
				JOptionPane.showMessageDialog(this,"테이블이 이미 존재합니다.");
				e.printStackTrace();
			}//catch
